...
import flask.cli #per sistemare errore compilazione 
import os
import random
import base64
...
# Percorso alla cartella temporanea (se in modalità onefile)
if hasattr(sys, '_MEIPASS'):
    BASE_DIR = sys._MEIPASS
else:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Directory reale accanto all’eseguibile
ROOT_DIR = os.path.dirname(os.path.abspath(sys.executable if getattr(sys, 'frozen', False) else __file__))

# Cartella reale per gli upload audio e la riproduzione
REAL_AUDIO_PATH = os.path.join(ROOT_DIR, 'audio')

# Se non esiste, copiala dalla versione embedded
if not os.path.exists(REAL_AUDIO_PATH):
    ...
    if os.path.exists(embedded_audio_path):
        ...
        os.makedirs(REAL_AUDIO_PATH, exist_ok=True)  # crea comunque cartella vuota

# Creazione dell'app FLASK
app = Flask(__name__)
app.config['SECRET_KEY'] = 'gmprojectorsecret123'
app.config['UPLOAD_FOLDER'] = os.path.join(ROOT_DIR, 'uploads')
#app.config['AUDIO_FOLDER'] = os.path.join(ROOT_DIR, 'audio')
app.config['AUDIO_FOLDER'] = os.path.join(BASE_DIR, 'audio')

# Configura Flask per usare il percorso reale
app.config['AUDIO_FOLDER'] = REAL_AUDIO_PATH


# Percorso base per PyInstaller
if getattr(sys, 'frozen', False):
    BASE_DIR = sys._MEIPASS  # directory temporanea creata da pyinstaller
...
def get_audio_moods():
    """Ottiene le cartelle (moods) disponibili nella directory audio"""
    moods = []
    ...
    if os.path.exists(audio_path):
        for item in os.listdir(audio_path):
            ...
            if os.path.isdir(mood_path):
                # Conta i file audio nella cartella
...
def get_random_audio_from_mood(mood):
    """Ottiene un file audio casuale da un mood specifico"""
...
@app.route('/')
def backend():
    """Pagina di controllo backend"""
    return render_template('backend.html')

@app.route('/frontend')
def frontend():
    """Pagina di visualizzazione frontend"""
...
def get_ip():
    ...
    try:
        # cerca di connettersi a Google DNS solo per ottenere la propria IP
...
def upload_image():
    """Upload e visualizzazione immediata dell'immagine"""
    ...
    if file and allowed_file(file.filename, ALLOWED_IMAGE_EXTENSIONS):
        ...
        # Aggiorna lo stato e notifica il frontend
...
@app.route('/clear_portraits')
def clear_portraits():
    socketio.emit('clear_portraits')
    return jsonify({'success': True})


@app.route('/clear_screen')
def clear_screen():
    """Pulisce lo schermo del frontend"""
...
@app.route('/get_moods')
def get_moods():
    """Ottiene la lista dei mood audio disponibili"""
...
def play_mood(mood):
    ...
    filename = random.choice(files)  # <--- solo nome del file
    socketio.emit('play_audio', {'mood': mood, 'audio': filename})  # 👍 CORRETTO
...
def upload_audio():
    ...
    if allowed_file(file.filename, {'mp3', 'wav', 'ogg'}):
        ...
        # Invia al frontend il comando di riproduzione
...
@socketio.on('connect')
def handle_connect():
    """Gestisce le nuove connessioni WebSocket"""
...
def handle_disconnect():
    """Gestisce le disconnessioni WebSocket"""
    print('Client disconnected')
    

@app.route('/shutdown')
def shutdown():
    # Scollega i client frontend
    socketio.emit('shutdown', namespace='/')

    # Chiude la finestra del browser con il backend (via JavaScript nel browser)
    return """
    <html>
